...
const Predicter = () => {
  ...
  const [numToPredict, setNumToPredict] = useState(20);
  ...
  const handleMinusOneWindow = (event) => {
    event.preventDefault();
    let newWindow = window - 1 < 0 ? 0 : window - 1;
    setWindow(newWindow);
  };

  const handlePlusOneWindow = (event) => {
    event.preventDefault();
    setWindow(window + 1);
  };
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 md:grid-rows-1">
                    ...
                    <fieldset>
                      ...
                      <div className="mt-5 space-y-4">
                        ...
                        <div className="flex items-start">
                          <div className="flex items-center h-5">
                            <input
                              id="predict-irrelevant"
                              name="Irrelevant"
                              type="checkbox"
                              checked={toPredict["Irrelevant"]}
                              onChange={handleToPredictChange}
                              className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded"
                            />
                          </div>
                  ...
                  <div className="px-4 py-5 bg-white sm:p-6">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm text-gray-500">
                        How many jobs to predict
                      </p>
                      <div className="flex flex-row h-10 rounded-lg bg-transparent mt-5">
                        <button
                          onClick={(e)=>{e.preventDefault();setNumToPredict(numToPredict-1)}}
                          className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50"
                        >
                          <span className="sr-only">Previous</span>
                          <svg
